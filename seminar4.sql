...
INSERT INTO `profiles` (user_id, gender, birthday, photo_id, hometown) VALUES 
   ...
   -- получить список пользователей, которые отправили сообщение
SELECT u.*, m.* -- users.*, messages
FROM users u -- users=u, левая
-- INNER JOIN =JOIN
JOIN messages m
ON u.id = m.from_user_id
-- + мод. сообщения только для 8 пользователя   
WHERE u.id =8;    

-- получить все сообщения
SELECT u.*, m.* -- users.*, messages
FROM messages m -- users=u, левая
-- INNER JOIN =JOIN
LEFT JOIN users u
ON u.id = m.from_user_id;
...
SELECT 
	id,
    CONCAT(firstname,' ',lastname) AS "Пользователь", -- склеили имя и фамилию через пробел
...
-- выбрать фотографии(filename) юзера с почтой arlo50@example.org
SELECT filename
FROM media
...
-- список медиафайлов всех пользователей с кол-вом лайков
SELECT
	m.id,
    ...
    FROM media m
    LEFT JOIN likes l 
    ON l.media_id =m.id -- список всех медиа и лайков к ним->
...
WHERE profiles.birthday>(NOW() - INTERVAL 12 YEAR);

-- определить кто поставил больше лайков мужчины или женщины
SELECT IF((SELECT COUNT(*) 
FROM users 
LEFT JOIN profiles on users.id=profiles.user_id 
LEFT JOIN likes ON users.id=likes.user_id WHERE profiles.gender="f")>(SELECT COUNT(*) 
FROM users LEFT JOIN profiles on users.id=profiles.user_id 
LEFT JOIN likes ON users.id=likes.user_id WHERE profiles.gender="m"),"больше лайков от женщин", "больше лайков от мужчин") AS fm;
